...
### v0.3.0
- 🖥️ **tkinter UI 界面**：图形化视频列表，双击即可下载
- 🎯 **嗅探全部开关**：复选框控制仅嗅探当前视频或全部视频
- 📂 **自定义保存目录**：输入框+浏览按钮，实时修改下载路径
- 🔧 **修复网络按钮**：一键关闭系统代理，恢复网络连接
- 🔌 **端口占用自动清理**：启动时检测并清理残留 mitmdump 进程
- 🔄 **跨进程 IPC 通信**：子进程写独立 JSON 文件，主进程轮询读取
- 🛡️ **单个模式去重**：按 description 去重，避免推荐列表重复嗅探
- 📝 **调试日志增强**：完整嗅探链路日志追踪
